"""
ridge_32.py  –  Lab 3.2 Part 2: Ridge Regression for BERT embeddings
=====================================================================
Identical bootstrap CV pipeline to ridge_31.py — only the input
design matrices and output paths differ.
 
Run from repo root:
    python code/lab32/ridge_32.py
"""
...
OUT_DIR = os.path.join(_RESULTS, 'ridge_32')
...
DM_DIRS = {
    'bert_pretrained': _DM_BASE,
    'bert_finetuned':  _DM_BASE,
    'bert_lora':       _DM_BASE,
}
 
# ── hyperparameters (identical to ridge_31.py) ───────────────────────────────
SUBJECTS   = ['subject2', 'subject3']
EMBEDDINGS = ['bert_pretrained', 'bert_finetuned', 'bert_lora']
...
def load_X_dense(emb: str, subject: str, split: str) -> np.ndarray:
    """Load one dense BERT design matrix for a subject/split combination."""
    dm_dir = DM_DIRS[emb]
...
def build_Y_memmap(stories: list, subject: str, tag: str) -> str:
    """
    Stack trimmed BOLD responses into an on-disk array.

    The response matrices are much larger than the design matrices, so this
    avoids holding the full train/test response stack in memory while building.
    """
...
def zscore_memmap_inplace(mmap_path: str, vchunk: int = 2000) -> np.ndarray:
    """Z-score each voxel time series in chunks to keep memory usage bounded."""
...
def corr_stats(corrs: np.ndarray) -> dict:
    """Summarize voxel-wise held-out CCs for tables and plotting scripts."""
    return {
...
def run(subject: str, emb: str) -> dict:
    ...
    # Standardize features using training-story statistics only; applying the
    # same mean/std to test stories prevents information leakage.
    ...
    # bootstrap_ridge selects a regularization value per voxel from held-out
    # chunks of the training stories, then evaluates the fitted weights on the
    # fixed held-out test stories.
